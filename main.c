...
int main(void){
    ...
    while(1){
        read_input();    // update input value

    // reset output ports
        GPIO_PORTN_DATA_R = 0x00;
        GPIO_PORTF_DATA_R = 0x00;

        if(!(GPIO_PORTJ_DATA_R & 0x01)) GPIO_PORTN_DATA_R |= 0x02; // PJ0 -> PN1 (Onboard LED D1)
        if(!(GPIO_PORTJ_DATA_R & 0x02)) GPIO_PORTN_DATA_R |= 0x01; // PJ1 -> PN0 (Onboard LED D2)
        if(!(GPIO_PORTM_DATA_R & 0x02)) GPIO_PORTF_DATA_R |= 0x10; // PM1 -> PF4 (Onboard LED D3)
        if(!(GPIO_PORTM_DATA_R & 0x01)) GPIO_PORTF_DATA_R |= 0x01; // PM0 -> PF0 (Onboard LED D4)
    }
}